...
def get_PMU(queue_out, config):
    
    global stopThread
    # Set up an instance of the PMU Sampled Value receiver
    
    svReceiver = mqttSVsub.Subscriber(config)  
    
    while not stopThread:      
        
        try:
            dataInfo = svReceiver.receive()    # Receive the latest frame of Sampled Values from the ADC.
        except:
            continue
...
if __name__ == '__main__':
    ...
    stopThread = False
    pmuQueue = Queue(3000)    
    t = Thread(target=get_PMU, args=(pmuQueue, config))
    t.start()
